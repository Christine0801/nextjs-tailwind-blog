...
lastmod: 2026-05-26 13:58:30
tags:
  - Don't Starve
  - backup
  - Server
draft: false
summary: "在 Linux 云服务器上使用一个名为 `z.sh` 的二进制程序（实际为 ELF 可执行文件）来启动《饥荒联机版》（Don't Starve Together）专用服务器。`z.sh` 提供了一个交互式菜单，可以手动启动/停止服务器。需求是：当服务器进程结束时，自动备份存档，并按存档名称分类保存，保留最近 10 个备份，同时记录清晰的日志。"
